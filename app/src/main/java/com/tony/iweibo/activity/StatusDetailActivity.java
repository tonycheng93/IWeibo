...
import com.tony.iweibo.adapter.StatusCommentAdapter;
...
import com.tony.iweibo.entity.response.CommentsResponse;
...
import com.tony.iweibo.widget.WrapHeightGridView;
...
public class StatusDetailActivity extends BaseActivity implements
    ...
    private StatusCommentAdapter mStatusCommentAdapter;
    ...
    private void loadComments(final long requestPage) {
        weiBoApi.commentsShow(mStatus.getId(), requestPage,
                new SimpleRequestListener(this, null) {
                    ...
                    public void onComplete(String response) {
                        ...
                        CommentsResponse commentsResponse = mGson.fromJson(response, CommentsResponse.class);
                        // 更新评论数信息
    ...
    private void addData(CommentsResponse response) {
        ...
        mStatusCommentAdapter.notifyDataSetChanged();
        // 用条数判断,当前评论数是否达到总评论数,未达到则添加更多加载footView,反之移除
        if (mComments.size() < response.getTotal_number()) {
            addFootView(plv_status_detail, footView);
        } else {
    ...
    private void setData() {
        ...
        mImageLoader.displayImage(user.getProfile_image_url(), iv_avatar,
                ImageOptHelper.getAvatarOptions());
        tv_subhead.setText(user.getName());
        tv_caption.setText(DateUtils.getShortTime(mStatus.getCreated_at() +
                " 来自" + Html.fromHtml(mStatus.getSource())));
        setImages(mStatus, include_status_image, gv_images, iv_image);

        if (TextUtils.isEmpty(mStatus.getText())) {
            tv_content.setVisibility(View.GONE);
        } else {
            tv_content.setVisibility(View.VISIBLE);
            SpannableString weiboContent = StringUtils.getWeiboContent(
                    this, tv_content, mStatus.getText());
            tv_content.setText(weiboContent);
        }

        Status retweetedStatus = mStatus.getRetweeted_status();
        ...
        // shadow_tab - 顶部悬浮的菜单栏
        shadow_rb_retweets.setText("转发 " + mStatus.getReposts_count());
        shadow_rb_comments.setText("评论 " + mStatus.getComments_count());
        shadow_rb_likes.setText("赞 " + mStatus.getAttitudes_count());
        // listView headerView - 添加至列表中作为header的菜单栏
        rb_retweets.setText("转发 " + mStatus.getReposts_count());
        rb_comments.setText("评论 " + mStatus.getComments_count());
        rb_likes.setText("赞 " + mStatus.getAttitudes_count());

        // bottom_control - 底部互动栏,包括转发/评论/点赞
        tv_share_bottom.setText(mStatus.getReposts_count() == 0 ?
                "转发" : mStatus.getReposts_count() + "");
        tv_comment_bottom.setText(mStatus.getComments_count() == 0 ?
                "评论" : mStatus.getComments_count() + "");
        tv_like_bottom.setText(mStatus.getAttitudes_count() == 0 ?
                "赞" : mStatus.getAttitudes_count() + "");
    }

    private void setImages(Status status, ViewGroup vgContainer,
        ...
        if (picUrl != null && picUrls.size() == 1) {
            vgContainer.setVisibility(View.VISIBLE);
            gvImgs.setVisibility(View.GONE);
            ivImg.setVisibility(View.VISIBLE);
            mImageLoader.displayImage(picUrl, ivImg, ImageOptHelper.getImgOptions());
        } else if (picUrl != null && picUrls.size() > 1) {
            vgContainer.setVisibility(View.VISIBLE);
    ...
    private void initView() {
        ...
        //listView --下拉刷新控件
        initListView();
        //bottom_control --底部互动栏，包括转发、评论、赞
        initControlBar();
    }
    ...
    private void initListView() {
        ...
        mStatusCommentAdapter = new StatusCommentAdapter(this, mComments);
        plv_status_detail.setAdapter(mStatusCommentAdapter);
        //footView --加载更多
